...
const getMusicMetadata = async () => import('music-metadata')
...
const normalizeFfmpegTimestamp = (value) => {
    const timestamp = String(value || '')
    if (!/^\d{2}:\d{2}:\d{2}(?:\.\d+)?$/.test(timestamp)) {
        throw new Boom('Invalid ffmpeg timestamp format (expected HH:MM:SS)')
    }
    return timestamp
}

const normalizeThumbnailWidth = (value) => {
    const width = Number(value)
    if (!Number.isFinite(width) || width <= 0) {
        throw new Boom('Invalid thumbnail width for ffmpeg')
    }
...
const extractVideoThumb = async (path, destPath, time, size) => new Promise((resolve, reject) => {
    try {
        const safeTime = normalizeFfmpegTimestamp(time)
        const safeWidth = normalizeThumbnailWidth(size?.width)
        const args = [
            '-ss', safeTime,
            '-i', String(path),
            '-y',
            '-vf', `scale=${safeWidth}:-1`,
            '-vframes', '1',
            '-f', 'image2',
            String(destPath)
        ]
...
function extensionForMediaMessage(message) {
    const getExtension = (mimetype) => mimetype?.split(';')[0]?.split('/')[1] || 'bin'
